...
public sealed class GitHubExternalSyncHandler : ExternalSyncHandlerBase
{
    private readonly IHabitLoggingService _habitLoggingService;

    public GitHubExternalSyncHandler(IHabitLoggingService habitLoggingService)
    {
    ...
    protected override async Task HandleCurrentAsync(ExternalSyncContext context, CancellationToken cancellationToken)
    {
        var items = context.ActivityItems
            .Where(x => x.Source == ExternalActivitySource.GitHub && x.ActivityDate == context.Date && x.Quantity > 0)
            .ToList();
        ...
        var habits = context.AutoSyncHabits
            .Where(h => h.AutoCompleteFromExternal && h.ExternalSource == ExternalActivitySource.GitHub)
            .ToList();
